...
use App\Mail\Elecciones\CandidaturaComentarioMail;
...
class SendCandidaturaComentarioEmailJob implements ShouldQueue
{
    ...
    {
        Log::info("Iniciando SendCandidaturaComentarioEmailJob", [
            'email' => $this->email,
        ...
        try {
            ...
            $mail = new CandidaturaComentarioMail($this->userName, $this->candidaturaId, $this->comentario);
            Mail::to($this->email)->send($mail);
            
            Log::info("Notificación de comentario de candidatura enviada exitosamente a {$this->email}", [
                'email' => $this->email,
                'candidatura_id' => $this->candidaturaId
            ]);
            ...
            if ($this->isRateLimitError($e)) {
                Log::info("Rate limit alcanzado enviando notificación de comentario a {$this->email}. El email será reintentado automáticamente.", [
                    'email' => $this->email,
                    'candidatura_id' => $this->candidaturaId,
                ...
                Log::error("Error SMTP enviando notificación de comentario a {$this->email}: " . $e->getMessage());
                throw $e;
            }
        } catch (\Exception $e) {
            Log::error("Error enviando notificación de comentario a {$this->email}: " . $e->getMessage());
    ...
    public function failed(\Throwable $exception): void
    {
        ...
        if ($exception instanceof \Symfony\Component\Mailer\Exception\UnexpectedResponseException 
            ...
            Log::warning("Job de notificación de comentario falló definitivamente por rate limiting para {$this->email}. " .
            ...
            Log::error("Job de notificación de comentario falló definitivamente para {$this->email}", [
                'email' => $this->email,
                'candidatura_id' => $this->candidaturaId,
