...
use App\Mail\Elecciones\CandidaturaPendienteMail;
...
class SendCandidaturaPendienteEmailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $mail = new CandidaturaPendienteMail($this->userName, $this->candidaturaId);
            Mail::to($this->email)->send($mail);
            
            Log::info("Notificación de candidatura pendiente enviada exitosamente a {$this->email} mediante job en cola", [
                'email' => $this->email,
            ...
            if ($this->isRateLimitError($e)) {
                Log::info("Rate limit alcanzado enviando notificación de candidatura pendiente a {$this->email}. El email será reintentado automáticamente.", [
                    'email' => $this->email,
                    'candidatura_id' => $this->candidaturaId,
                ...
                Log::error("Error SMTP enviando notificación de candidatura pendiente a {$this->email} mediante job: " . $e->getMessage());
                throw $e;
            }
        } catch (\Exception $e) {
            Log::error("Error enviando notificación de candidatura pendiente a {$this->email} mediante job: " . $e->getMessage());
    ...
    public function failed(\Throwable $exception): void
    {
        ...
        if ($exception instanceof \Symfony\Component\Mailer\Exception\UnexpectedResponseException 
            ...
            Log::warning("Job de envío de notificación de candidatura pendiente falló definitivamente por rate limiting para {$this->email}. " .
            ...
            Log::error("Job de envío de notificación de candidatura pendiente falló definitivamente para {$this->email}: " . $exception->getMessage());
        }
    }
