...
class SendCandidaturaReminderWhatsAppJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            if (!$this->isPhoneFormatValid($this->phone)) {
                Log::warning("Número de teléfono con formato inválido, no se enviará recordatorio de candidatura", [
                    'phone' => $this->phone,
            ...
            $message = $this->getCandidaturaReminderMessageTemplate($this->userName);
            
            // Enviar mensaje por WhatsApp
            $sent = $whatsappService->sendMessage($this->phone, $message);
            
            if ($sent) {
                Log::info("Recordatorio de candidatura enviado exitosamente por WhatsApp", [
                    'phone' => $this->phone,
                    'candidatura_id' => $this->candidaturaId
                ]);
            } else {
    ...
    protected function handleSendFailure(): void
    {
        ...
        $errorMessage = "No se pudo enviar el recordatorio de candidatura por WhatsApp";
        ...
        Log::error("No se pudo enviar el recordatorio de candidatura por WhatsApp", [
            'phone' => $this->phone,
            'candidatura_id' => $this->candidaturaId,
            'attempt' => $this->attempts()
        ]);
    ...
    protected function handleException(\Exception $e): void
    {
        ...
        Log::error("Error temporal enviando recordatorio de candidatura por WhatsApp", [
            'phone' => $this->phone,
            'candidatura_id' => $this->candidaturaId,
            'error' => $message,
            'attempt' => $this->attempts(),
    ...
    {
        Log::error("Job de recordatorio de candidatura WhatsApp falló definitivamente", [
            'phone' => $this->phone,
            'candidatura_id' => $this->candidaturaId,
    ...
    /**
     * Obtener plantilla de mensaje para recordatorio de candidatura
     */
    protected function getCandidaturaReminderMessageTemplate(string $userName): string
    {
        return "Hola {$userName},\n\n" .
               "Te recordamos que tu candidatura está en estado *borrador* y aún no ha sido completada.\n\n" .
               "Para completar tu candidatura y enviarla para revisión, ingresa a la plataforma en:\n" .
               config('app.url') . "\n\n" .
               "Es importante que completes todos los campos requeridos antes de la fecha límite.\n\n" .
               "¿Necesitas ayuda? Contáctanos a soporte@colombiahumana.co\n\n" .
               "Saludos,\n" .
               "Colombia Humana";
    }
}