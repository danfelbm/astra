...
use App\Mail\Elecciones\CandidaturaBorradorMail;
...
class SendCandidaturaBorradorEmailJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $mail = new CandidaturaBorradorMail($this->userName, $this->candidaturaId, $this->motivo);
            Mail::to($this->email)->send($mail);
            
            Log::info("Notificación de candidatura devuelta a borrador enviada exitosamente a {$this->email}", [
                'email' => $this->email,
            ...
            if ($this->isRateLimitError($e)) {
                Log::info("Rate limit alcanzado enviando notificación de vuelta a borrador a {$this->email}. El email será reintentado automáticamente.", [
                    'email' => $this->email,
                    'candidatura_id' => $this->candidaturaId,
                ...
                Log::error("Error SMTP enviando notificación de vuelta a borrador a {$this->email}: " . $e->getMessage());
                throw $e;
            }
        } catch (\Exception $e) {
            Log::error("Error enviando notificación de vuelta a borrador a {$this->email}: " . $e->getMessage());
    ...
    public function failed(\Throwable $exception): void
    {
        ...
        if ($exception instanceof \Symfony\Component\Mailer\Exception\UnexpectedResponseException 
            ...
            Log::warning("Job de notificación de vuelta a borrador falló definitivamente por rate limiting para {$this->email}. " .
            ...
            Log::error("Job de notificación de vuelta a borrador falló definitivamente para {$this->email}", [
                'email' => $this->email,
                'candidatura_id' => $this->candidaturaId,
