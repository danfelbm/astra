...
import { useDebounce } from "@modules/Core/Resources/js/composables/useDebounce";
...
const breadcrumbs: BreadcrumbItemType[] = [
    { title: 'Inicio', href: '/miembro/dashboard' },
    { title: 'Asambleas', href: '/miembro/asambleas' },
...
// Campos básicos predefinidos que siempre deben estar disponibles
...
const filterFieldsConfig = ref<any[]>([...defaultFilterFields]);
...
const filterConfig = computed<AdvancedFilterConfig>(() => {
    // Asegurar que los campos básicos siempre estén primero
    // Primero los campos básicos (nombre, documento), luego los del backend, luego los geográficos
    const backendFields = filterFieldsConfig.value.filter(
        f => !defaultFilterFields.some(df => df.name === f.name)
    );
    
    const allFields = [
        ...defaultFilterFields,  // Siempre primero: nombre, documento
        ...backendFields,       // Otros campos del backend
        ...geographicFilters.generateFilterFields(),  // Campos geográficos al final
    ];
...
const fetchParticipantes = async (page = 1, filters = {}) => {
    ...
    try {
        const response = await axios.get(route('user.asambleas.participantes', props.asamblea.id), {
            params: {
                page,
                ...filters,
            },
        ...
        // Actualizar configuración de filtros del backend si viene algo diferente
        if (response.data.filterFieldsConfig && response.data.filterFieldsConfig.length > 0) {
            // Combinar con los campos por defecto sin duplicar
            const newFields = response.data.filterFieldsConfig.filter(
                field => !defaultFilterFields.some(df => df.name === field.name)
            );
            filterFieldsConfig.value = [...defaultFilterFields, ...newFields];
        }
    } catch (error) {
...
const registrarAsistencia = async (participanteId: number, asistio: boolean) => {
    ...
    try {
        const response = await axios.put(
            route('user.asambleas.marcar-asistencia-participante', {
...
// Volver al listado
const volver = () => {
    router.visit(route('user.asambleas.index'));
};
