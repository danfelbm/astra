...
namespace Modules\Asamblea\Repositories;
...
class AsambleaRepository
{
    ...
    private function countSincronizados(Asamblea $asamblea, $votacion): int
    {
        return \DB::table('votacion_usuario')
            ...
            ->where('model_type', 'Modules\Asamblea\Models\Asamblea')
            ->count();
    }
     ...
     */
    public function getAsambleasForUser(\Modules\Core\Models\User $user, Request $request, int $perPage = 18): LengthAwarePaginator
    {
     ...
     */
    private function transformAsambleaForUser($asamblea, \Modules\Core\Models\User $user, array $asambleasParticipanteIds): array
    {
     ...
     */
    public function getParticipantesForUser(Asamblea $asamblea, \Modules\Core\Models\User $user, Request $request): array
    {
    ...
    public function searchPublicParticipant(Asamblea $asamblea, string $search): array
    {
        ...
        $participante = \Modules\Core\Models\User::query()
            ->join('asamblea_usuario', 'users.id', '=', 'asamblea_usuario.usuario_id')
            ->where('asamblea_usuario.asamblea_id', $asamblea->id)
