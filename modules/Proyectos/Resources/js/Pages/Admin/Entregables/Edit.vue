...
// Ya no necesitamos parseFecha - usaremos strings directamente

// Form data - inicializar con datos del entregable
const form = useForm({
...
props.usuariosAsignados.forEach(asignado => {
  if (!usuariosCache.value.has(asignado.user_id)) {
    // Si el usuario no est\u00e1 en la lista, crear un placeholder
...
const submit = () => {
  ...
  form.transform(() => dataToSend).put(`/admin/proyectos/${props.proyecto.id}/hitos/${props.hito.id}/entregables/${props.entregable.id}`, {
    onSuccess: () => {
...
const estadoInfo = computed(() => {
  if (props.entregable.estado === 'completado' && props.entregable.completado_at) {
    const fecha = new Date(props.entregable.completado_at).toLocaleDateString('es-ES');
    const usuario = props.entregable.completado_por?.name || 'Usuario desconocido';
  ...
  <AdminLayout :breadcrumbs="breadcrumbs">
    <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
      ...
      <Card>
        ...
        <CardContent>
          <div class="grid gap-4 md:grid-cols-3">
            ...
            <div>
              <p class="text-sm text-muted-foreground">Estado del Hito</p>
              <Badge>{{ hito.estado_label || hito.estado }}</Badge>
            </div>
            <div>
              <p class="text-sm text-muted-foreground">Progreso del Hito</p>
      ...
      <form @submit.prevent="submit" class="space-y-4">
        ...
        <Card>
          ...
          <CardContent class="space-y-4">
            ...
            <div>
              ...
              <div v-if="showUsuariosAsignados" class="space-y-4 p-4 border rounded-lg">
                <div class="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader>
            <CardTitle>Notas de actualización</CardTitle>
            <CardDescription>
              Describe los cambios realizados (opcional)
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Textarea
        ...
        <div class="flex justify-end gap-2">
          ...
          <Button type="submit" :disabled="!canSubmit">
            ...
            {{ form.processing ? 'Actualizando...' : 'Actualizar Entregable' }}
          </Button>
        </div>
      </form>
    </div>
