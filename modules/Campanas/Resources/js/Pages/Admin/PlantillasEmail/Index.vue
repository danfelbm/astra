...
import { Edit, Plus, Search, Trash2, Mail, Copy, Eye, FileText, Calendar, User, CheckCircle, XCircle } from 'lucide-vue-next';
...
const filterConfig: AdvancedFilterConfig = {
    ...
    quickSearchFields: ['nombre', 'descripcion', 'asunto'],
    maxNestingLevel: 2,
...
// Aplicar filtros
const applyFilters = () => {
    router.get('/admin/campanas/plantillas-email', {
...
const duplicatePlantilla = (id: number) => {
    router.post(`/admin/campanas/plantillas-email/${id}/duplicate`, {}, {
        onSuccess: () => {
...
const deletePlantilla = (id: number) => {
    router.delete(`/admin/campanas/plantillas-email/${id}`, {
        onSuccess: () => {
...
const previewPlantilla = (plantilla: PlantillaEmail) => {
    router.post(`/admin/campanas/plantillas-email/${plantilla.id}/preview`, {}, {
        onSuccess: (response: any) => {
            // Abrir preview en modal o nueva ventana
            window.open(`data:text/html;charset=utf-8,${encodeURIComponent(response.html)}`, '_blank');
        }
    });
};
    ...
    <AdminLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
            ...
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-3xl font-bold">Plantillas de Email</h1>
                    <p class="text-muted-foreground mt-1">
                        Gestiona las plantillas de correo electrónico para tus campañas
                    </p>
                </div>
                <Link 
                    v-if="canCreate"
                    :href="'/admin/campanas/plantillas-email/create'"
                    class="inline-flex"
                >
                    <Button>
                            ...
                            <TableRow v-for="plantilla in plantillas.data" :key="plantilla.id">
                                ...
                                <TableCell>
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm">{{ plantilla.asunto }}</span>
                                    </div>
                                </TableCell>
                                <TableCell>
                                ...
                                <TableCell class="text-right">
                                    <div class="flex justify-end gap-1">
                                        ...
                                        <Link
                                            v-if="canEdit"
                                            :href="`/admin/campanas/plantillas-email/${plantilla.id}/edit`"
                                        >
                            ...
                            <TableRow v-if="plantillas.data.length === 0">
                                <TableCell colspan="7" class="text-center py-8">
                                    <div class="text-muted-foreground">
                                        ...
                                        <p>No se encontraron plantillas de email</p>
                                    </div>
                                </TableCell>
                            </TableRow>
                        </TableBody>
