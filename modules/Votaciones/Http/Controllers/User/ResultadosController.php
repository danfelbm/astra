...
class ResultadosController extends UserController
{
    ...
    public function show(Votacion $votacion): Response
    {
        ...
        return Inertia::render('Modules/Votaciones/User/Resultados', [
            'votacion' => [
                'id' => $votacion->id,
    ...
    public function consolidado(Votacion $votacion)
    {
        ...
        foreach ($formularioConfig as $pregunta) {
            ...
            if (in_array($pregunta['type'], ['select', 'radio', 'convocatoria', 'perfil_candidatura'])) {
                ...
                // Asegurar que todos los valores sean strings para countBy
                $respuestasParaContar = $respuestasProcesadas->map(function($item) {
                    if (is_array($item)) {
                ...
                $conteos = $respuestasParaContar->countBy();
                ...
                if (in_array($pregunta['type'], ['convocatoria', 'perfil_candidatura'])) {
                    ...
                    foreach ($conteos as $opcion => $cantidad) {
                        // Si la opción es JSON, decodificarla para mostrar
                        $opcionDisplay = $opcion;
                        if (str_starts_with($opcion, '[') || str_starts_with($opcion, '{')) {
                            $decoded = json_decode($opcion, true);
                            if ($decoded !== null && is_array($decoded)) {
                                $opcionDisplay = is_array($decoded) ? implode(', ', $decoded) : $opcion;
                            }
                        }
                    ...
                    // Verificar que options existe y es un array
                    if (isset($pregunta['options']) && is_array($pregunta['options'])) {
                        foreach ($pregunta['options'] as $opcion) {
                        // Si la opción es un array, usar su valor o label
                        $opcionKey = is_array($opcion) ? ($opcion['value'] ?? $opcion['label'] ?? json_encode($opcion)) : $opcion;
                        
                        // Convertir la opción a string para buscar en conteos
                        $opcionKeyStr = is_array($opcionKey) ? json_encode($opcionKey) : (string) $opcionKey;
                        
                        $cantidad = $conteos->get($opcionKeyStr, 0);
                        $porcentaje = $totalVotos > 0 ? round(($cantidad / $totalVotos) * 100, 2) : 0;
                        
                        $preguntaData['respuestas'][] = [
                            'opcion' => is_array($opcion) ? ($opcion['label'] ?? $opcionKey) : $opcion,
                            'cantidad' => $cantidad,
                            'porcentaje' => $porcentaje,
                        ];
                        }
                ...
                // Asegurar que todos los valores sean strings para countBy
                $opcionesParaContar = collect($todasLasOpciones)->map(function($item) {
                    if (is_array($item)) {
                        return json_encode($item);
                    }
                    return (string) $item;
                ...
                $conteos = $opcionesParaContar->countBy();
        ...
        return response()->json([
            ...
            \Log::error('Error en consolidado:', [
            ...
            return response()->json([
                'error' => 'Error al procesar resultados consolidados',
                'message' => config('app.debug') ? $e->getMessage() : 'Por favor intenta nuevamente más tarde',
