...
namespace Modules\Elecciones\Http\Controllers\User;
...
class CandidaturaController extends UserController
{
    /**
    ...
    public function index()
    {
        ...
        if (!$config || !$config->tieneCampos()) {
            return Inertia::render('Modules/Elecciones/User/Candidaturas/NoDisponible', [
        ...
        return Inertia::render('Modules/Elecciones/User/Candidaturas/Dashboard', [
    ...
    public function create()
    {
        ...
        if ($configuracionBloqueo['bloqueo_activo']) {
            ...
            return Inertia::render('Modules/Elecciones/User/Candidaturas/Bloqueado', [
                'titulo' => $configuracionBloqueo['bloqueo_titulo'],
                'mensaje' => $configuracionBloqueo['bloqueo_mensaje'],
                'tipo' => 'crear',
            ]);
        ...
        if ($candidaturaExistente) {
            return redirect()->route('user.candidaturas.edit', $candidaturaExistente);
        }
        ...
        if (!$config || !$config->tieneCampos()) {
            return redirect()->route('user.candidaturas.index')
                ->with('error', 'No hay configuración de candidaturas disponible.');
        }

        return Inertia::render('Modules/Elecciones/User/Candidaturas/Form', [
            'candidatura' => null,
        ...
        try {
            ...
            if ($convocatoria) {
                ...
                if (!Postulacion::usuarioTienePostulacion($usuario->id, $convocatoria->id)) {
                    ...
                    \Modules\Elecciones\Models\PostulacionHistorial::crearRegistro(
                        $postulacion,
    ...
    public function show(Candidatura $candidatura)
    {
        ...
        return Inertia::render('Modules/Elecciones/User/Candidaturas/Show', [
            'candidatura' => [
                'id' => $candidatura->id,
    ...
    public function update(Request $request, Candidatura $candidatura)
    {
        ...
        if ($estadoFinal === Candidatura::ESTADO_PENDIENTE && $nuevoEstado === Candidatura::ESTADO_PENDIENTE) {
            ...
            if (!$postulacionExistente) {
                ...
                if ($convocatoriaId) {
                    ...
                    $convocatoria = \Modules\Elecciones\Models\Convocatoria::find($convocatoriaId);
        ...
        return redirect()->route('user.candidaturas.index')->with('success', $mensaje);
    }
    ...
    private function validarFormularioDinamico(Request $request, array $campos)
    {
        ...
        $conditionalService = new \Modules\Formularios\Services\ConditionalFieldService();
