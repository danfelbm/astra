...
namespace Modules\Elecciones\Http\Controllers\Admin;

use Modules\Core\Http\Controllers\AdminController;
use Modules\Elecciones\Jobs\SendCandidaturaReminderEmailJob;
use Modules\Elecciones\Jobs\SendCandidaturaReminderWhatsAppJob;
use Modules\Elecciones\Models\Candidatura;
use Modules\Elecciones\Models\CandidaturaConfig;
use Modules\Elecciones\Models\CandidaturaCampoAprobacion;
use Modules\Elecciones\Models\CandidaturaComentario;
...
use Modules\Core\Traits\HasSegmentScope;
...
class CandidaturaController extends AdminController
{
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.view'), 403, 'No tienes permisos para ver candidaturas');
        ...
        $candidaturas->through(function ($candidatura) use ($totalCampos, $convocatoriaFieldId) {
            ...
            $comentarioActual = CandidaturaComentario::where('candidatura_id', $candidatura->id)
                ->where('version_candidatura', $candidatura->version)
                ->orderBy('created_at', 'desc')
                ->first();
            
            // Contar total de comentarios históricos
            $totalComentarios = CandidaturaComentario::where('candidatura_id', $candidatura->id)
                ->count();
        ...
        return Inertia::render('Modules/Elecciones/Admin/Candidaturas/Index', [
    ...
    public function show(Candidatura $candidatura)
    {
        ...
        return Inertia::render('Modules/Elecciones/Admin/Candidaturas/Show', [
            'candidatura' => [
                'id' => $candidatura->id,
                'usuario' => [
                    'id' => $candidatura->user->id,
            ...
            'puede_comentar' => Auth::user()->can('candidaturas.comment'),
        ]);
    }
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.approve'), 403, 'No tienes permisos para aprobar candidaturas');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.reject'), 403, 'No tienes permisos para rechazar candidaturas');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.comment'), 403, 'No tienes permisos para agregar comentarios');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.view'), 403, 'No tienes permisos para ver comentarios');
    ...
    public function configuracion()
    {
        ...
        return Inertia::render('Modules/Elecciones/Admin/Candidaturas/Configuracion', [
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.view'), 403, 'No tienes permisos para ver configuración');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.view'), 403, 'No tienes permisos para ver estadísticas');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.edit'), 403, 'No tienes permisos para editar candidaturas');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.view'), 403, 'No tienes permisos para ver historial');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.aprobar_campos'), 403, 'No tienes permisos para aprobar campos individuales');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.aprobar_campos'), 403, 'No tienes permisos para rechazar campos individuales');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.view'), 403, 'No tienes permisos para ver estado de aprobación');
    ...
    {
        // Verificar permisos
        abort_unless(auth()->user()->can('candidaturas.edit'), 403, 'No tienes permisos para enviar recordatorios');
